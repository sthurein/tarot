...
💬 <b>အကူအညီလိုပါက / ငွေသွင်းပြီးပါက:</b>
 09444456145 ကိုဖုန်းဆက် အကြောင်းကြားပေးပါခင်ဗျာ။
"""

CLOSED_MSG = """
🙏 မင်္ဂလာပါဗျာ...

ယခုအချိန်သည် ည ၉ နာရီမှ မနက် ၉ နာရီအထိ ဟောခန်းပိတ်ချိန်ဖြစ်ပါသည်။ ဆရာ့ထံတွင် <b>မနက် ၉း၀၀ နာရီ မှ ည ၉း၀၀ နာရီ (မြန်မာစံတော်ချိန်)</b> အတွင်းသာ အသစ် Booking ယူခြင်းများ ဆောင်ရွက်နိုင်ပါတယ်ခင်ဗျာ။

မနက်ဖြန်မှ ပြန်လည် ဆက်သွယ်ပေးပါဗျ။
"""
...
# ================= 4. DATABASE, FREE PERIOD & TIME LOGIC =================
...
def is_operating_hours():
    now = mm_now()
    # 9 AM မှ 9 PM (21:00) အတွင်းဖြစ်ပါက True ပြန်ပေးမည်
...
def send_welcome(message):
    ...
    # 1. Booking ရှိပြီးသားလား အရင်စစ်မယ် (ရှိရင် အချိန်မရွေး ဝင်ခွင့်ပေးမယ်)
    if check_subscription(user_id):
        ...
        # 2. Booking မရှိဘူးဆိုရင်၊ အခုက ဆိုင်ဖွင့်ချိန် ဟုတ်မဟုတ် စစ်မယ်
        if not is_operating_hours():
            bot.reply_to(message, CLOSED_MSG, parse_mode="HTML")
            return

        # 3. ဆိုင်ဖွင့်ချိန်ဆိုရင်တော့ ပုံမှန်အတိုင်း Free ကာလ/ငွေလွှဲတောင်းတာတွေ ဆက်လုပ်မယ်
        if is_free_period():
...
def handle_slip(message):
    ...
    # ငွေလွှဲပြေစာပို့ရင် Booking အရင်ရှိမရှိ စစ်မယ် (မရှိမှ အချိန်စစ်မယ်)
...
def handle_message(message):
    ...
    # Access စစ်ဆေးခြင်း
    if not check_subscription(user_id):
        # Booking မရှိဘူးဆိုရင်၊ အခုက ဆိုင်ဖွင့်ချိန် ဟုတ်မဟုတ် စစ်မယ်
        if not is_operating_hours():
            bot.reply_to(message, CLOSED_MSG, parse_mode="HTML")
            return
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Critical Error: {e}")model = genai.GenerativeModel('gemini-flash-latest')
